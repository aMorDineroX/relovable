...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/openApi/swap/v1/user/commissionRate?${queryString}&signature=${signature}`, {
      headers: {
        'X-BX-APIKEY': API_KEY,
    ...
    if (!response.ok) {
      return NextResponse.json({
        success: false,
        error: data.msg || 'Failed to fetch commission rates',
        code: data.code
